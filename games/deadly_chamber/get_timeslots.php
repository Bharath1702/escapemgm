...
if($date > $current_date){
    ...
    $sql = "SELECT id, time FROM timeslots WHERE time > '$current_time' AND id NOT IN (SELECT timeslot_id FROM deadly_chamber WHERE date = '$date')";
}elseif($current_date>$date){
    echo"<script>echo('Select a valid date')</script>";
