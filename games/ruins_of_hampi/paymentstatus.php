...
if ($responsePayment['success'] && $responsePayment['code'] == "PAYMENT_SUCCESS") {
    ...
    $stmt = $conn->prepare("INSERT INTO ruins_of_hampi (name, email, mobile, date, no_of_players, timeslot_id, txnID) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'Booking Failed for Ruins Of Hampi';
        $mail->Body = "
        <!DOCTYPE html>
        <html lang='en'>
        <head>
        ...
        ";
        $mail->AltBody = 'Booking Unsuccessful for Ruins Of Hampi. Check your email for details.';
